...
public class MylynGitHubApi implements GitHubApi {
	 ...
	 * @throws IOException
	 */
	private void createObviousFixCommentIfNecessary(PullRequestId pullRequestId, String userMentionMarkdown, IssueService issues, List<Comment> claUserComments) throws IOException {
	...
	public Set<String> findAssociatedClaNames(String repoId, String accessToken) {
		try {
			...
			//
			//
			hooks.stream().filter(h -> StringUtils.hasText(h.getConfig().get("url"))).filter(RepositoryHook::isActive).map(h -> h.getConfig().get("url")).filter(PULL_REQUEST_CALLBACK_PATTERN.asPredicate()).map(url -> getClaName(url, PULL_REQUEST_CALLBACK_PATTERN)).collect(Collectors.toSet());
			return claNames;
		} catch (final java.lang.Throwable $ex) {
			throw lombok.Lombok.sneakyThrow($ex);
		}
